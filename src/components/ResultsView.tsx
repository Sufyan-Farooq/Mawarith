    ...
    <div className="space-y-6">
      ...
      <div className="relative rounded-2xl bg-obsidian-900 text-white p-5 sm:p-6 border border-white/[0.08] shadow-float overflow-hidden">
        ...
        <div className="relative z-10 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div>
            <h2 className="results-heading-rule">{t.resultsTitle}</h2>
        ...
        <div className="relative z-10 grid grid-cols-2 sm:grid-cols-4 gap-3 mt-5 pt-4 border-t border-white/[0.08] text-xs">
          <div className="p-2.5 rounded-xl bg-white/[0.03] border border-white/[0.05]">
            <span className="block text-obsidian-400 text-xs">{t.grossEstate}</span>
          ...
          <div className="p-2.5 rounded-xl bg-white/[0.03] border border-white/[0.05]">
            <span className="block text-rose-300/80 text-xs">{t.debtsAndBurial}</span>
          ...
          <div className="p-2.5 rounded-xl bg-white/[0.03] border border-white/[0.05]">
            <span className="block text-brass-300/80 text-xs">{t.wasiyyahDeduction}</span>
          ...
          <div className="p-2.5 rounded-xl bg-jade-950/60 border border-jade-600/40">
            <span className="block text-jade-300 text-xs font-semibold">{t.netInheritable}</span>
        ...
        {stackedItemsList.length > 0 && (
          <div className="relative z-10 mt-4 pt-3 border-t border-white/[0.08] text-xs">
            <button
              ...
              className="w-full flex items-center justify-between text-xs font-semibold text-obsidian-300 hover:text-white transition-colors"
            >
      ...
      {heirs.length > 0 && (
        <div className="p-4 rounded-2xl bg-white border border-slate-200/90 shadow-micro space-y-2">
          <div className="flex items-center justify-between text-xs text-obsidian-700">
            ...
            <span className="font-mono text-xs text-obsidian-500">100% Shariah Compliant</span>
          </div>
          ...
          <div className="h-3 w-full bg-slate-100 rounded-lg overflow-hidden flex shadow-inner">
            {heirs.map((h, i) => (
              <motion.div
                ...
                className="h-full"
                title={`${getHeirDisplayName(h, language)}: ${h.percentage.toFixed(1)}%`}
              >
              </motion.div>
            ))}
          </div>
          ...
          <div className="flex flex-wrap items-center gap-3 pt-1">
            {heirs.map((h, i) => (
              <div key={h.heirId} className="flex items-center gap-1.5 text-xs text-obsidian-600">
                <span
                  className="w-2 h-2 rounded-full shrink-0"
                  style={{ backgroundColor: colorPalette[i % colorPalette.length] }}
                />
                <span>{getHeirDisplayName(h, language)}</span>
                <span className="font-mono text-obsidian-500">({h.percentage.toFixed(1)}%)</span>
              </div>
            ))}
          </div>
        </div>
      ...
      <div className="rounded-2xl bg-white border border-slate-200/90 shadow-float overflow-hidden">
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-xs sm:text-sm">
            <thead>
              <tr className="bg-slate-50/80 text-xs text-obsidian-500 uppercase tracking-wider border-b border-slate-100">
                  ...
                  <tr key={h.heirId} className="hover:bg-slate-50/70 transition-colors">
                    ...
                    <td className="px-4 py-3 text-start">
                      <div className="font-bold text-obsidian-900 flex items-center gap-1.5">
                        <span>{getHeirDisplayName(h, language)}</span>
                        {h.count > 1 && (
                          <span className="text-xs font-medium text-obsidian-500">
                            (×{h.count})
                          </span>
                        )}
                      </div>
                      {h.customNames && h.customNames.filter(Boolean).length > 0 && (
                        <div className="text-xs font-medium text-jade-800 mt-0.5">
                          {h.customNames.filter(Boolean).join('، ')}
                        </div>
                      )}
                      <span className="text-xs text-obsidian-500 block mt-0.5">
                    ...
                    <td className="px-4 py-3 text-end">
                      ...
                      {h.count > 1 && (
                        <span className="text-xs text-obsidian-500 font-mono block">
      ...
      <div className="rounded-2xl bg-slate-50/80 border border-slate-200/80 p-4 space-y-3">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2 text-xs font-bold text-obsidian-700">
            ...
            <span className="text-xs font-mono bg-white px-1.5 py-0.5 rounded border border-slate-200 text-obsidian-500">
              {blockedHeirs.length}
            </span>
          </div>
        </div>
